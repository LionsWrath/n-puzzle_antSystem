...
void createLeaf(Session * session, Node * node, 
    ...
    if (!isTableInHash(leaf->table, session->tables) || isFinal(session, leaf)) {
...
int calculateProbability(Session * session, Node * node) {
    int i = 0, j;
    float uppers[4];
    float lower = 0;
    ...
    for (i = 0; i < node->numberOfLeafs; i++) {
        ...
        if (isFinal(session->endTable, leaf->table)) return i; 

        uppers[i]  = pow(leaf->pheromone, 
                session->alpha); 
    ...
    int max = getMax(node, probability); 
...
int updatePheromone(float rho, Node * node, int Q) {
    int pathSize = node->height;

    do {
        node->pheromone = node->pheromone*(1.0f - rho) + Q/pathSize;
        node = node->father;
    } while (node != NULL);
}

bool applyConstruction(Session * session, Node * node) {
    int chosen, Q = INITIAL_Q;
  
    //O que acontece quando um nó não tem filhos
    while (!isFinal(session->endTable, node->table) && Q != 0) {
        printf("Altura: %d\n", node->height);
        if (0 == node->numberOfLeafs) generateLeafs(session, node);
        if (0 != node->numberOfLeafs) {
            printf("NOL: %d\n", node->numberOfLeafs);
            chosen = calculateProbability(session, node);
            node = node->leafs[chosen];
        } else {
            Q = 0;
        }
    }
    ...
    if (Q == 0) {
        updatePheromone(session->rho + .1, node, Q);
        return false;
    } else {
        updatePheromone(session->rho, node, Q);
        return true;
    }
}
...
void findBest(Session * session, Node * root) {
    //Buscar melhor solução encontrada
}
